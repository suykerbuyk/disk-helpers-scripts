...
LOGDIR="piknik_$(date --iso-8601).draid2x10x42.rec_size=512k_with_spec_64k"
...
#zpool add altlabs01 -f -o ashift=12 special \
#	mirror /dev/disk/by-id/nvme-WUS3BA138C7P3E3_A05A135F /dev/disk/by-id/nvme-WUS3BA138C7P3E3_A06463B1 \
#	mirror /dev/disk/by-id/nvme-WUS3BA138C7P3E3_A064B05B /dev/disk/by-id/nvme-WUS3BA138C7P3E3_A06F104F
...
create_stripe_zpool() {
	wipe_zfs_pools
	zpool create ${POOL_NAME}  -O recordsize=64k -O atime=off -O dnodesize=auto -o ashift=12 ${LUN_PATTERN}
	for X in $(ls -lah /dev/disk/by-id/wwn-0x600c0ff00* | grep -v part | awk -F '/' '{print $7}')
	do
		echo 8192 >/sys/block/$X/queue/max_sectors_kb
	done
			...
			for PAT in 'read' 'write' 'randrw' 'randread' 'randwrite'; do
				...
				for BLK in 1048576 524288 262144 131072 32768 16384  ; do
					...
					do
						...
						TESTFS="${POOL}/TEST/FIXED512k_SPEC_VDEV_${BLK_NAME}_64k_spec_vdev"
						...
						zfs create -p -o recordsize=512k -o compression=off ${TESTFS}
						zpool wait -t initialize ${POOL}
						zfs set special_small_blocks=64k ${TESTFS}
						zpool wait -t initialize ${POOL}
						echo "Running $TEST"
						./fio-3.30 --directory=/${TESTFS} \
						    --name="${TEST}" \
						    --size=1024G \
						    --rw=$PAT \
						    --group_reporting=1 \
