...
#LUN_PATTERN="/dev/disk/by-id/scsi-SSEAGATE_ST18000NM004J*"
LUN_PATTERN="/dev/disk/by-id/nvme-WUS3BA*"
LOGDIR=piknik_$(date --iso-8601).four.nvme_concurrent_blk_devs
		...
		for JOBS in 1 4 8; do
			for PAT in 'write' 'read' 'randrw' 'randread' 'randwrite'; do
				for BLK in 4096 8192 16384 32768 131072 262144 524288 1048576; do
					...
					FILENAME="$(ls ${LUN_PATTERN} | grep -v part | head -50 | tr '\n' ':')"
					FILENAME="${FILENAME::-1}"
					TEST="four_nvme_block-${IOENGINE}-${IODEPTH}-${PAT}-${BLK}-${JOBS}.fio.json"
					echo "Running $TEST"
